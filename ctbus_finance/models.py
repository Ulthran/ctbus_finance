from sqlalchemy import (
    Column, Integer, String, Float, ForeignKey, UniqueConstraint, Date, DateTime
)
...
class Account(Base):
    ...
    name = Column(String, nullable=False, primary_key=True)  # e.g., "My Brokerage Account"
...
class AccountHolding(Base):
    ...
    __table_args__ = (
        UniqueConstraint('account_id', 'holding_id', 'date', 'purchase_date', name='uix_account_holding_date'),
    )

    account_id = Column(String, ForeignKey('accounts.name'), primary_key=True, nullable=False)
    holding_id = Column(String, ForeignKey('holdings.symbol'), primary_key=True, nullable=False)
    ...
    purchase_price = Column(Float, nullable=True)  # Price at which the asset was purchased
    ...
    percentage_large_cap = Column(Float, nullable=True)  # Percentage of large cap stock in the holding
    percentage_mid_cap = Column(Float, nullable=True)  # Percentage of mid cap stock in the holding
    percentage_small_cap = Column(Float, nullable=True)  # Percentage of small cap stock in the holding
    percentage_international = Column(Float, nullable=True)  # Percentage of international stock in the holding
    percentage_other = Column(Float, nullable=True)  # Percentage of other assets in the holding
...
class CreditCardHolding(Base):
    ...
    credit_card_id = Column(String, ForeignKey('credit_cards.name'), primary_key=True, nullable=False)
    date = Column(Date, primary_key=True, nullable=False)
