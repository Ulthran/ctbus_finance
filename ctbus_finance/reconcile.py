...
def get_account_balance(txns: list[data.Transaction], account: str) -> list[position.Position]:
...
def reconcile_transaction(txn: data.Directive, index: int, txns: data.Directives) -> data.Directive:
    if "todo" in txn.meta:
        print(f"ADDRESSING TODO: {txn.meta['todo']}")

        if txn.meta.get("todo_type", "") == "DISTRIBUTION" and type(txn) == data.Transaction:
            ...
            others = [t for t in txns[:index-1] if type(t) == data.Transaction and acct in [p.account for p in t.postings]]
            ...
            for p in positions:
                postings.append(data.Posting(
                    account=acct,
                    units=-p.units,
                    cost=position.Cost(p.cost.number, p.cost.currency, None, None) if p.cost else None,
                    price=None,
                    flag=None,
                    meta=None,
                ))
                postings.append(data.Posting(
                    ...
                    cost=position.Cost(p.cost.number / ratio, p.cost.currency, None, None) if p.cost else None,
                    price=None,
                    flag=None,
                    meta=None,
                ))

            meta = {k: v for k, v in txn.meta.items() if k != "todo" and k != "todo_type"}
            if ratio > 1:
